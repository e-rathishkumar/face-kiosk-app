...
enum _CaptureStep {
  ...
  left('left', 'Turn Left', Icons.arrow_back_rounded),
  right('right', 'Turn Right', Icons.arrow_forward_rounded),
...
class _FaceCapturePageState extends State<FaceCapturePage>
  ...
  bool _isCorrectPosition(_CaptureStep step, double rotY, double rotX) {
    switch (step) {
      ...
      case _CaptureStep.up:
        return rotX > 7;   // Positive rotX = face looking up
      case _CaptureStep.down:
        return rotX < -7;  // Negative rotX = face looking down
    }
  }
  ...
  Future<void> _uploadAll() async {
    ...
    final result = await authRepo.register360Face(authState.user.id, captures);
